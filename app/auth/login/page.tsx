    ...
    <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#1A1A1A] tracking-tight">Добро пожаловать</h1>
          <p className="text-sm text-[#9A9A9A] mt-1">Войди в аккаунт</p>
        </div>
        ...
        <div className="bg-white border border-[#E8E8E4] rounded-2xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[#1A1A1A] mb-1.5">Никнейм или телефон</label>
              <input type="text" value={login} onChange={e => setLogin(e.target.value)}
                placeholder="username или +77XXXXXXXXX" required
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm font-medium text-[#1A1A1A]">Пароль</label>
                <Link href="/auth/reset-password" className="text-xs text-[#2563EB] hover:underline">Забыл пароль?</Link>
              </div>
        ...
        <p className="text-center text-sm text-[#9A9A9A] mt-4">
          Нет аккаунта?{' '}
          <Link href="/auth/register" className="text-[#2563EB] font-medium hover:underline">Зарегистрироваться</Link>
        </p>
      </div>
    </div>
  )
